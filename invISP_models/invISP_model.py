...
class InvISPNet(nn.Module):
    def __init__(self, channel_in=3, channel_out=3, subnet_constructor=subnet('DBNet'), block_num=8):
        ...
        # self.first_operation = InvBlock(subnet_constructor, channel_num, channel_split_num) # one block is one flow step.
    ...
    def forward(self, x, rev=False):
        ...
        if not rev:
            # out_1 = self.first_operation(out,rev)
            # out = out_1
            ...
            for op in reversed(self.operations):
                out = op.forward(out, rev)
            # out = self.first_operation(out,rev)
