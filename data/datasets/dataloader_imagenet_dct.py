...
import time
import torch
from data.datasets import ImageFolderDCT
from data import datasets as transforms
from data.datasets import train_y_mean, train_y_std, train_cb_mean, train_cb_std, \
    train_cr_mean, train_cr_std
from data.datasets import train_y_mean_upscaled, train_y_std_upscaled, train_cb_mean_upscaled, train_cb_std_upscaled, \
    train_cr_mean_upscaled, train_cr_std_upscaled
from data.datasets import train_dct_subset_mean, train_dct_subset_std
from data.datasets import train_upscaled_static_mean, train_upscaled_static_std
from data.datasets import train_upscaled_static_dct_direct_mean, train_upscaled_static_dct_direct_std
from data.datasets import train_upscaled_static_dct_direct_mean_interp, train_upscaled_static_dct_direct_std_interp
...
def trainloader_upscaled_static(args, model='mobilenet'):
    ...
    transform = transforms.Compose([
        ...
        transforms.RandomHorizontalFlip(),
        transforms.Upscale(upscale_factor=2),  # required for the following networks
...
def trainloader_dct_resized(args):
    ...
    train_dataset = ImageFolderDCT(traindir, transforms.Compose([
        ...
        transforms.RandomHorizontalFlip(),
        transforms.TransformDCT(),  # 28x28x192  # losslessly read the DCT
