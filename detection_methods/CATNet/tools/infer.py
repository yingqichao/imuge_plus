...
def main():
    ...
    ## CHOOSE ##
    args = argparse.Namespace(cfg='/ssd/ImageForensicsOSN/CATNet/experiments/CAT_full.yaml', opts=['TEST.MODEL_FILE', '/groupshare/CATNet/RGB_only_v2.tar', 'TEST.FLIP_TEST', 'False', 'TEST.NUM_SAMPLES', '0'])
    ...
    ## CHOOSE ##
    test_dataset = splicing_dataset(crop_size=None, grid_crop=True, blocks=('RGB', ), DCT_channels=1, mode='arbitrary', read_from_jpeg=True)  # full model
    ...
    with torch.no_grad():
        for index, (image, label, qtable) in enumerate(tqdm(testloader)):
            ...
            p_h, p_w = pred.shape
            pred_large = cv2.resize(pred, (p_w*4, p_h*4)).astype(np.uint8)
            # filename
            filename = os.path.splitext(get_next_filename(index))[0] + "1.png"
            filepath = dataset_paths['SAVE_PRED'] / filename
            cv2.imwrite(str(filepath), pred_large)
