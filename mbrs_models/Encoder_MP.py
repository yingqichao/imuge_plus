...
class Encoder_MP(nn.Module):
	...
	def __init__(self, out_channels=3, blocks=4, channels=64):
		...
		self.after_concat_layer = ConvBNRelu(channels, channels)
	...
	def forward(self, image):
		...
		intermediate3 = self.after_concat_layer(intermediate1)

		# skip connection
		if self.out_channels == 3:
			concat2 = torch.cat([intermediate3, image], dim=1)
		else:
			concat2 = intermediate3
