...
from inpainting_methods.saicinpainting.evaluation.data import InpaintingDataset as InpaintingEvaluationDataset, \
    ceil_modulo
from inpainting_methods.saicinpainting.training.data.aug import IAAAffine2, IAAPerspective2
from inpainting_methods.saicinpainting.training.data.masks import get_mask_generator
...
def make_default_val_dataset(mask, image, kind='default', out_size=512, transform_variant='default', **kwargs):
    ...
    dataset = InpaintingEvaluationDataset(mask, image, **kwargs)
    return dataset

# def make_default_val_dataset(mask, image, kind='default', out_size=512, transform_variant='default', **kwargs):
#     # if OmegaConf.is_list(indir) or isinstance(indir, (tuple, list)):
#     print("make_..._dataset")
#     return ConcatDataset([
#         make_default_val_dataset_concat(mask[i], image[i], kind=kind, out_size=out_size, transform_variant=transform_variant, **kwargs) for i in range(mask.shape[0]) 
#     ])
