...
if __name__ == '__main__':
    ffc_net = FFC_BN_ACT(64, 64, kernel_size=3, padding=1, norm_layer=nn.BatchNorm2d,
                         activation_layer=nn.ReLU, ratio_gin=0.5, ratio_gout=0.5)
    input_tensor = torch.rand(1, 64, 32, 32)
    input_tensor = torch.chunk(input_tensor, chunks=2, dim=1)
    x_l, x_g = ffc_net(input_tensor)
    print(x_l.shape)
    print(x_g.shape)
    ## output is torch.Size([1, 32, 32, 32]) * 2
